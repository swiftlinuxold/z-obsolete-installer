...
class PerformInstall(threading.Thread):
	...
	def run(self):
		try:				
			...
			self.report("Removing this installer from the target system")
			...
			dialog = gtk.MessageDialog(None, gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_NONE, _("Swift Linux was successfully installed on your computer. You can now eject the CD and reboot the system."))
...
class mainWindow:
	...
	self.wTree.get_widget("label_summary_partition").set_text(_("Swift Linux will be installed in <b>%s</b>") % (value))
	...
	try: 
        ...
        dlg.set_authors(["Jason Hsu swiftlinux@jasonhsu.com"]) 
